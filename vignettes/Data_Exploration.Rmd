...
To begin to familisarise ourselves with the data, it is very useful to be able examine summary information, including min, mean, and max values for each covariate that you have laoded. Environmental covariate data is subject to error. For example, maybe the maximum value in your elevation variable is 1000 m, but you know that the highest peak in your study region is only 500 m. During the data entry process someone may have added an extra 0 to a 100 m measurement by mistake. Maybe your vegetation classification is showing as having ten levels despite it being an eight-category scale. Chances are a spelling mistake as benign as 'forest' instead of 'Forest' is the culprit. 
...
Carolina_Wren_Workflow <- workflow(occurrence = test,
...
You can check for collinearity in many ways, but the simplest is to look at a pair plot of your covariate data. You can do this using the `zoon` output module `PairPlot`. Variable names are listed down the diagonal, upper panel shows the scatterplots of relationships between covariates, the diagonal panel shows a histogram of the locations of points, and the lower panel shows the $r^2$ correlation values. 
...
We can manipulate our data directly by using the accessor function `Process()` to generate a dataframe object with extracted raster covariate values for each occurrence point. We can examine the first six rows of this data frame by using the base R function `head()`. 
