...
With our basic workflow loaded, we can begin to examine our occurrence and covariate data in more detail by calling the accessor function `Process`, which will display the combined occurrence data with extracted values of the raster covariates. 
...
When we first begin to examine our data, we will want to check for a few key properties, so that our coding and modelling work as expected. First, we want to check that our data contains the data we expect and that each variable is stored in the right format. Categorical variables should be stored as factors, however they may be listed, for example, in a numerical index (e.g. vegetation categories identified as 1-10 instead of 'VegClass1', 'VegClass2', etc) and therefore interpreted as numerical data. Similarly, if there are any typos in numerical data entries that introduce characters (e.g. '2 cm' instead of '2'), then a continuous variable will be classed as a factor. 

We can check both the data included as well as their format with the `str()` function, or check individual variables with `class()`. 
...
If can also help to visualise the spread of your variables, to more easily idetify unusual or unreasonable values. One way you can do this is with the `Outliers` module. 
...
many of the models will require that you run the model on the same data. and will require you to remove NA values beforehand. You can only compare models that are built on the same set of data. This is because the out-of-sample variance will be higher in the model that is calculated with more data and therefore its AIC will be higher. 
...
## Relationships
A startin gstep in your analysis might be to explore how the 
