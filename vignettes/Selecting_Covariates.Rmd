...
We will start by setting up a basic `zoon` `workflow()` with the Sugar Maple data and using raster covariate dataset from Bioclim for illustration.  We will set `process = NoProcess`, `model = NullModel`, and `output = PrintOccurrenceMap` - this map simply shows us our raw occurrence points (in red) and our background points (in black), no model has been applied. 
...
+ Skewed/systematically varying residuals: Transformation does not just change the distribution of the raw data, it also implies a relationship of the residuals. If there is a pattern among the residuals ('heterscedasticity') then transformation can be used to remove that pattern (ensuring 'homoscedasticity'), an assumption of some models. 
