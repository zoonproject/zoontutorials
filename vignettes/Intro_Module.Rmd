...
Species distribution models estimate the probabilty of occurrence of species along environmental gradients. They do this by estimating the correlation between enviornmental variables and species occurrence data. For example, using SDMs, we can estiamte the probability a species of bird will use forest habitat with high tree cover compared to open habitat with low tree cover. Or we might estimate the probability of occurrense for a frog across the a whole continent using climatic variables, such as rainfall and temperature. We can take these statisical relationships and project them onto geographic space, which allows us to visualise on a map how the probabilty of species occurrence varies in geographic space.

There are many types of species distribution models and the field is rapidly expanding. `zoon` is concerned with correlative SDMs, as described above, which are the most widley used SDM. This tutoiral will focus on models which estimate probability of occurrence, however, we could also use species abundance data to estimate how abundance varies along environemntal gradients. As well as correlative SDMs, there are also mechanistic SDMs. Where correlative SDMs use statiscs to estimate relatinoships between occurreance data and environemntal covariates, mechanistic SDMs use mathematical relationships between species energy requirements and environmental covaraites. Where correlative SDMs estimate a species realised niche, mechanistic SDMs estimates the fundemental niche. The field of mechanistic models is ... and we encourage the read to pursue their interest further, as we leave the topic here. 
...
```{r message=FALSE, warning=FALSE, fig.align='center', fig.height=7, fig.width=7, cache=TRUE}
...
```{r eval=TRUE, message=FALSE, warning=FALSE, fig.show=FALSE, cache=TRUE}
zoon_workflow <- ChangeWorkflow(workflow = zoon_workflow,
                                process = Chain(OneHundredBackground, StandardiseCov))
```
...
`########################`
Code for Liz to modify as needed for changing output modules in zoon_workflow. Need to change eval to TRUE, currently FALSE so doc can knit. While you could use List to do multiple outputs at once, it might be better to step through them one by one with multiple ChangeWorkflows? Or split into another best pratice guide?

```{r eval=FALSE, warning=FALSE, message=FALSE, fig.align='center', fig.height=7, fig.width=7, cache=TRUE}
zoon_workflow <- ChangeWorkflow(workflow = zoon_workflow,
                                output = )
```
...
Now that we have seen how to use each argument in the `workflow` function it is time to put all of the pieces together and fit our own SDM within `zoon`. Here we fit a logistic regression model to Carolina Wren presence-background data set, generate 1000 background samples, and display our results in a map without our datapoints displayed. We have covered each of the necesssary modules previously, and here we will run them together to form a complete `workflow`.
