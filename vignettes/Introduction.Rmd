...
vignette: |
  %\VignetteEngine{knitr::rmarkdown} %\VignetteIndexEntry{Introduction to SDMs} %\VignetteEncoding{UTF-8}
---
...
Knowing where biota are in our world is important for a range of reasons. We might want to know where an endangered species is most likely to occur so we can protect its habitat. Or we might want to know where a disease will break out so we can prevent it from doing so. Or perhaps we'd like to know where a species of fish occurs so we can responsibly harvest our fisheries. We can answer all these questions and more using species distribitions models. 

Species distribution models predict species distributions by correlating environmental covariates, such as temperature or rainfall, with observations of species occurence - presence, absence, or abundance. While these relationships are correlative, as opposed to mechanistic, they still help us identify potential ecological relationships between species and environments. They enable us to make ecological inferences.

Environmental response curves, for example, illustrate how probabilty of species occurrence varies in *environmental space*, that is, over environmental gradients. Figure 1a shows an example of a response curve for the blue whale, *Balaenoptera musculus*. This plot shows how blue whale abundance varies along a gradient of sea surface height (SSH) [@redfern17].

These relationships, between species and environment, can be used to map the distribution of species in *geographic space*. Figure 1b shows a predicted distribtion for the Ebola virus [@pigott14]. Mapping the current distribution of a species can be useful for planning responses to public health emergencies or prioritising habitat conservation, for example.
...
## What?
Correlative species distribution models estimate species realised niches. They estimate the probabilty of species occurrence along environmental gradients by correlating occurrence data with environmental covariates. Let's revisit the ecological theory underlying species distributions.

A species distribution is typically considered the result of three factors: the abiotic environment, the biotic environment, and dispersal abillity [@soberon05]. The intersection of the abiotic and biotic environment constitutes the *fundamental niche*. This is where the species could *potentially* occur given what we know of its biology and ecology. However, most species do not occur everywhere they potentially could. Where a species *actually* occurs is termed the *realised niche*. For example, a realised niche could be constrained by the climate required for individuals to survive, the local vegetation types it depends on, and its abillity to traverse different landscapes. Species interactions are also considered to constrain the fundemental niche to the realised niche. For example, two closely related species may have mutally exclusive geophraphic distributions despite having similar physiological tolerances, habitat preferences, and dispersal ability.
...
This introductory guide will demonstrate how to use `zoon` to fit species distribution models. `zoon` is an R package designed to make fitting SDMs simple and reproducible. `zoon` keeps things simple by splitting the SDM process into five easy steps:
...
4. Fitted a MaxNet model and,
5. Generated a map as our output.

The printed map illustrates the Carolina wren's distributions as predicted by our SDM. 

We'll work with this workflow throughout this introductory guide. In order to tackle one step at a time, we'll update this workflow one argument at a time.
...
+  Presence-only data are usually sourced from museum or herbarium records, or citizen scientists, and consist of a list of observations of a species and the location of the observation.
...
Next it's time to consider any necessary data processing. We process our data by choosing modules for the `process` argument of `workflow()`. There's a whole suite of data processing we might want to do before fitting our model. For example, we could clean our data by removing poor data points using the `Clean` module. The process argument is also where we generate background data points for when we have a presence-only `occurrence` module. We can actually do both (or more) at once using the `Chain()` function! 
...
The process argument must be given a module, even when we don't need to process our data. In those instances, we can use the `NoProcess` module.
...
For now, let's work with a common modelling method - `LogisticRegression`. We'll keep our data and process modules the same:
...
Once we're happy with our data, modelling method, and data processing, it is finally time to check our results. `output` is the fifth and final argument of the `zoon` workflow function and `zoon` SDM fitting process.
...
The probability of Carolina wren occurrence increases along the bio1 gradient and decreases with bio3. bio2 seems to have little effect for the Carolina wren.
