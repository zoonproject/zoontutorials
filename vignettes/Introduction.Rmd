...
A basic workflow for the Joshua tree from California could look like this:
...
In this workflow, we have obtained occurrence records for *Yucca brevifolia* (Joshua Tree), loaded the Bioclim environmental covariates, selected 100 background occurrence data (more on this later), fit a RandomForest model, and printed a map. The map shows us how the probability of occurrence of *Yucca brevifolia* varies across south-eastern USA. There are multiple ways to visualise the output of a model and we will cover them in a later section.
...
<<<<<<< HEAD
# The five `zoon` workflow steps
=======
# The five `zoon` workflow steps *edit*
>>>>>>> f75462e2360b96da645039c57d578e54e3caf7ee
## Step 1. Occurrence
...
Using what we've learned here, lets update `zoon_workflow` so that we can build an SDM for the Carolina Wren in the USA. CarolinaWrenPo is an occurrence module that comes pre-loaded with presence only data for the Carlonia wrne ifor north america. As presence-only data is the most commonly used data type for SDMs lets use the presence-only version of the dataset using `CarolinaWrenPO`, and we get the environmental data from `CarolinaWrenRasters`.
...
While we have used some existing data modules in this tutorial, there are modules available that will obtain occurrence and environmental data for you from online repositories such as GBIF and Bioclim. Here we show you the difference between datasets for the Carolina Wren from the pre-loaded modules `CarolinaWrenPO` and `CarolinaWrenRasters` compared with datsets sourced from online repositories using the modules `SpOcc` and `Bioclim`. We already know the extent of the datsets from the pre-loaded modules (-138.71, -52.58, 18.15, 54.95) so we will use those values for the online repositories as well.
...
**paragraph on difference between two. Wait on discussion with others first **
...
Let's check out our environmental data by using the function `Covariate()`. This function returns a summary of our raster data. In this case, we have six covariates stored in raster layers which are themselves stored in a raster brick. Our covariates are the percentage of mixed (pcMix), deciduous (pcDec), and coniferous (pcCon) forests as well as the percentage of grassland (pcGr). We have also included latitude and longitude so we can quantify the changes in the species distribution in geographic space.
