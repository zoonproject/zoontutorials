...
For some modelling methods, it is helpful to standardise variables, thereby putting them on the same scale. Always check the assumptions of your selected model, however, as this is not always necessary. Many machine learning methods like random forests, boosted regression trees, and MaxEnt (**check MaxEnt. It rescales everything 0-1, so I assume insensitive?**) are insensitive to standardisation.
